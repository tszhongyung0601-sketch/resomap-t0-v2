...
- Bottom nav 四個 tab。（V5 起第四格是「更多優惠」，一起規劃搬進行程 tab；數量從未變過。）
- **地圖與我的不是 tab**：地圖在 BrandBar 漢堡選單，我的在 BrandBar 右上頭像。
- 沒有住在 shell 裡的浮動 AI 按鈕（T0 刻意移除過：一顆按鈕、四種行為、每頁都在）。
  行程那一頁後來自己長了一顆（見 25.8），差別是它只做一件事、只在一個分頁上。
...
- [x] Bottom nav 仍是四個（V5 起第四格為「更多優惠」）
...
**沒有新功能。** 所有東西都已經存在，這一輪決定的是它們住在哪裡。

### 25.1 一起規劃搬進行程

它不是被拿掉，是被搬到它本來就屬於的地方。跟朋友一起決定，是「對一趟行程做的事」；
把「行程」跟「討論這趟行程」放在畫面最下面的兩端，是把同一個主題切成兩半。

旅行文件同理：登機證屬於它對應的那趟行程旁邊，不是設定頁往下數第四列。

行程 tab 因此有三個分頁：**行程（預設）／文件／一起規劃**，沿用既有的
`Segmented` 元件，沒有做第二套分頁。

**清單維持第一、維持原樣。** 打開行程的人是來看自己的行程的，
一個逼他先讀懂一排控制項才能看到清單的分頁，等於先跟他收一筆他沒要求的費用。

### 25.2 第四格給了沒有家的那一半

空出來的位置給了這個產品裡真的沒有家的部分：68 家商家、42 位服務者、
5 個聯盟平台、22 個租車據點——原本只能捲到首頁最下面、在九個小圖示裡找。

**仍然是四個 tab。** V3 那條規矩是「不要變成 5 個、不要把『我的』放進 bar」，
兩條都還成立。

### 25.3 先分來源，再分類別——這就是整個設計

這兩群不是同一種東西，而讀的人有權知道自己在讀哪一種：

- **ResoMap 合作商家**：自己收的、經過審核、可以掛「推薦夥伴」（付費**並且**通過審核，
  兩個都要）。這是這家公司要負責的供給。
- **聯盟合作平台**：別人的庫存，ResoMap 跟每一家都沒有合作關係，`affiliateUrl` 全空。

如果照類別混排——住宿挨著住宿——就等於把這兩件事合成一份清單，
並且悄悄把「審核過」那一半的信用借給另一半。

### 25.4 蓋到一半抓到的：五顆會騙人的按鈕

第一版把商家那組寫成五列可點的 Row，每一列都開「優惠」畫面——
而那個畫面列的是 deal，不是商家。

這個 app **沒有全域的商家瀏覽器**，而且不該有：排序是「離你站的位置多遠」，
所以入口本來就必須從一個地點進去。五顆點下去會到不相干地方的按鈕，
正是這個專案一路在刪的那種死控制項。

改成純數字，加一句話講清楚要從哪裡看它們。

### 25.5 精靈刪掉，不是藏起來

`screens/AiPlanner.tsx`（V3 的五步驟）整支刪除，連同 `{ k: "aiPlan" }` route。
對話能做它做的每一件事，而且能改現有行程。留一支沒有入口的畫面就是死程式碼——
上一輪刪情境圖就是同一個理由。

`CreateTrip` 沒動（T0 的建立空行程精靈，四個入口共用，不是同一件事）。
`lib/planner.ts` 留著，對話在用它。

### 25.6 文件補上 eSIM 與住宿（第八輪追加）

原本只有登機證能被認出來，其他一律變成「🎫 文件」——住宿沒有自己的類別，
eSIM 根本不存在。

**eSIM 跟登機證一樣有國際標準。** GSMA SGP.22 定義的啟用碼是 `$` 分隔的定形字串：

```
LPA:1$rsp.example.com$QR-G-5C-1LC-1S6TG6N
│   │ └ SM-DP+ 伺服器 ┘ └ 啟用碼 ───────┘
│   └ 格式版本，目前恆為 1
└ scheme，很多 QR 直接從 1$ 開始，所以是選填
```

所以 `lib/lpa.ts` 是照規格切字，不是猜。掃到就自動歸成 eSIM，不會問使用者這是什麼。

**住宿沒有標準，所以用選的。** 掃完之後跳出「這是什麼？」讓使用者選住宿／eSIM／其他，
選住宿才出現飯店名稱、入住、退房、訂房代號四個欄位。

四種類別裡**兩種是認出來的、兩種是選的**，而這個分界不是隨便畫的：
有公開標準的就讀，沒有的就問。已經認出來的不給改——
讓一個下拉選單推翻 BCBP 的解析結果，等於允許記錄跟它自己的原始字串互相矛盾。

**啟用碼是憑證。** 它就是讓一台裝置能領走那個 eSIM 設定檔的東西。
所以範例是偽造的（`example.com` 是 RFC 2606 保留給文件用的網域），
而且畫面上那句「只存在這台裝置」對它比對登機證更重要。

**做的時候抓到兩個 bug：**

1. 掃到 eSIM 會自動跳出「補上資料」的表單——但它已經被讀出來了，
   跳一張表單問使用者已知的事情是多此一舉。
2. `ManualSheet` 沒有 `key`。開了 A 的表單再開 B 的，React 重用同一個實例，
   而每個 `useState` 只在首次掛載時初始化——所以 B 開起來顯示的是 A 的名稱、
   日期與類別，按下儲存就會把 A 的資料寫進 B。加上 `key={editing.id}`。


### 25.7 三個入口，一個「對到行程」（第八輪追加）

**掃描從一顆按鈕變成三顆。** 原本只有「掃描登機證 / QR Code」一個入口，
而人是拿著一個具體的東西走過來的——他手上是登機證、是 eSIM 的 QR、
還是訂房確認信。一顆合併的按鈕等於要他自己先想清楚這個 app 收不收他那一種。

類別只是**提示，而且維持是提示**。`addDoc` 仍然以字串真正的內容為準：
按了「掃住宿」而掃到的是登機證，它照樣被歸成登機證。
提示真正買到的是解析失敗的那一種——只是一串網址的飯店 QR，
現在一進來就已經標好「住宿」，該填的欄位也已經在等了，而不是落到「其他」。

**「對到行程」現在三種文件都有，但它們對到的不是同一件事。**
這裡本來有個陷阱：登機證的「對到行程」是「把落地那天整天往後移」，
而住宿跟 eSIM 對這件事**沒有任何可以推算的依據**。
把同一顆按鈕原封不動裝到它們身上，就是這個專案一路在刪的那種假按鈕。

所以拆成上下兩層：

- **上層：這份文件屬於哪一趟。** 三種都適用，也都真的有用——
  它讓「這趟的東西在哪」有答案，而不是一疊沒有日期的卡片。
  這一層按下去就寫（`linkDoc`），因為選中的 chip 就是回饋，再按一次就取消。
- **下層：把落地那天往後移。** 只有解析成功的登機證才有，
  而且要先選好行程才出現——沒有行程的時候它是一句說明，不是一顆按不動的按鈕。
  這一層維持「提案然後等」，跟這個 app 裡所有會改寫行程的東西一樣。

**行程頁多一列「🛂 旅行文件」**，數字只算對到這一趟的。
文件不會因為日期接近就自動被歸到某一趟：兩趟行程可以重疊、
一張訂房可以是別人的，而把登機證悄悄歸錯行程，是沒有人要求過的那種貼心。

零的時候那一列也在，寫的是「機票・住宿・eSIM」。
一個要先有東西才會出現的入口，等於只有已經知道它存在的人找得到。

順手改掉的一句話：套用預覽本來會寫「往後 0 小時 30 分」。

### 25.8 對話改成右下角的浮動按鈕（第八輪追加）

原本是行程清單上面的一列「💬 跟 AI 說你想怎麼玩」。它會動、也講得清楚，
但它花掉的是整個螢幕的寬度跟第一屏的頂端——而這一頁唯一的工作，
是讓人看到自己的行程。所以那一列換成右下角一顆浮動按鈕。

**這個 app 刪過一次浮動 AI 按鈕**，理由記在 `AppShell.tsx` 的註解裡：
一顆按鈕停在每一頁上，而且每一頁做的事都不一樣，所以沒有人學得會它是幹嘛的。
新的這顆是那件事的反面：

- **只做一件事**——開對話，沒有第二種行為。
- **臉上寫著那件事**——是「💬 跟 AI 說」的膠囊，不是一顆只有圖示的圓。
  只放一個 💬 等於把當年那個教訓裡「看不懂」的那一半原封不動留著。
- **只屬於一個分頁**——不在 shell 裡，所以 `AppShell.tsx` 根本不認識它。
  文件那頁的主要動作是掃描，一起規劃那頁是投票，而這個對話的 10 種 intent
  全部都是在講行程——它對登機證沒有話講，對投票也加不上什麼。

**位置是 `Screen` 的兄弟，不是它的小孩。** 螢幕會捲，這顆不能捲。
放在外面之後它的定位基準是 shell 的內容區，而那一塊的下緣剛好就是 tab bar 的上緣，
所以 `bottom-4` 在畫出來的手機上跟真手機上都是同一個結果，
這支檔案不需要知道 nav 有多高、home indicator 有多厚。

高度 52px（跟 `Button` 同一個 `h-13`），在 44px 的觸控下限之上。
底下留 `h-24` 的空白，最後一列不會是被壓在按鈕底下的那一列。

## 26. V6：真天氣，假活動（第九輪）

一輪裡兩個功能，而它們對「真」的處理剛好相反——這不是不一致，是同一條規矩
套在兩種資料上的結果：**能查證的就得是真的，不能查證的就得說清楚是編的。**

### 26.1 天氣：這個 app 第一個對外的網路請求

其他所有假資料都不吃虧：一個編出來的門票價格，仍然忠實示範了「門票價格長什麼樣」。
**天氣是唯一一個使用者轉個頭就能查證的數字。** Demo 現場外面在下雨而畫面寫「晴」，
那不叫示意，那叫在觀眾面前出錯。所以這一個接真的。

Open-Meteo：免金鑰、免帳號、CORS 開放、非商用免費——這是一個 GitHub Pages 上的
靜態頁能問到天氣的唯一原因。**後端還是沒有。**

**拿不到就什麼都不顯示。** 離線、被擋、被限流、格式不對，全部收斂成 `null`，
每個呼叫端都是「整段不畫」而不是畫一個佔位符。
該有溫度的地方空著是一個事實（我們不知道）；
不知道卻寫 25° 是一句謊。

**「預報」與「實測」分開標，因為它們不是同一種東西。**
預報是對未來的宣稱、可能會錯；過去那天是量測值、不會錯。
Demo 的行程日期在兩週前（`8 月 15 日`，沒有年份），所以畫面上多數是後者——
`past_days=92` 就是為了這個：只往前看的視窗，會讓這個功能在它隨附的行程上永遠空白。

三個位置：首頁地圖那條的即時氣溫（你站的位置）、行程總覽每張日卡、每日行程頁的標題列。
後兩者用**目的地**的座標，不是你現在的位置——一趟東京的行程不是在問新店。
一趟行程問一次，不是一天問一次。

### 26.2 活動：第一批連東西本身都不存在的資料

68 家商家是真店。22 個租車據點是真公司真地址。聯盟商品是假價格掛在真平台上。
那些情況裡，「Demo」標籤保護的是一個**數字**，而東西本身存在、去得到。
**這裡東西本身不存在。**「碧潭河岸夏夜市集」沒有在辦、從來沒辦過、沒有人在籌辦。

所以揭露不是標題旁一個 11px 的灰標籤，是一句話，在閱讀順序裡，
列表上一次、詳情頁再一次：**這些活動是編的。**
一個因為這個畫面而買車票去壽豐的人，是被這個 app 具體地誤導了，
而 11px 的「Demo 資料」不能免除這件事。

**刻意保持真的兩件事：**

- **地點。** 真行政區、真座標。沒有它們就算不出距離，而距離就是這個功能本身——
  「附近」是承諾，得對著某個地方量。
- **記錄的形狀。** 起訖日期、開始時間、收不收費、室內或戶外。
  真的活動資料源長這樣，所以將來真的接上時，畫面不用改。

**名字刻意避開真實節慶。** 平溪天燈節、鹽水蜂炮、大甲媽祖遶境、宜蘭童玩節
都是有真實主辦單位的真實活動，把編的日期與票價掛在那些名字上，
比憑空取一個名字更糟：那是把不實資訊塞進一件真實的事情裡。

**日期不存年份。** `[8, 15]` 是八月十五，是哪一年在讀取時才決定。
不然每年都要有人手動重新標日期，而年份會是第一個被忘記的東西。

### 26.3 兩個日期問題，兩種算法，刻意分開

- **「這是哪一次？」** 需要年份，取「還沒結束的下一次」。用來算還有幾天、用來排序。
  往回找一年（跨年的活動現在正在辦，但起始日在去年），往前找四年（2 月 29 日）。
- **「這一天在不在檔期裡？」** 完全不需要年份，而且**不可以**發明一個。
  行程的某一天是「8 月 16 日」，也沒有年份。兩邊都用月日比，
  不是偷懶，是唯一有定義的比法——先各自解析成年份，
  會出現一邊落在 2026、一邊落在 2027，然後回報兩個明明重疊的日期不重疊。

26 個測試（`npm run test:events`）押在這一段上，因為這是整個功能裡
唯一會**安靜地錯**的部分：跨年、閏日、單日活動、月底跨月、
以及「正在進行中的活動不該回報成明年」。

### 26.4 「加入行程」是這個 app 第一個可以真的不存在的按鈕

餐廳、租車、導遊——任何一天都排得進去。
一場只辦 16 號的活動排不進 18 號，而一份包含「去不了的那一站」的行程，
比沒有按鈕更糟。所以日期對不上時，按鈕的位置放的是**理由**，
而且寫出兩邊的日期——「加不進去」而不說為什麼，是一個使用者無法查證的指控。

`AddToTrip` 因此多了一個 `days` 參數：由呼叫端算出哪幾天能去，
sheet 只負責不要提供其他的。省略就是全部，這對「只是一個地點」的東西才是對的。

城市對不上（新店的市集 vs 花蓮的行程）**不擋**，只講：
日期是使用者定的規則，而人可能真的當晚飛回來。
但一份安靜地把兩個城市塞進同一個下午的行程，只會在車站才現形，
所以那句話印在按鈕之前，不是按下之後才發現。

### 26.5 首頁那條只顯示真的在附近的，所以另外要有全部

排序是「公里 + 天數」相加，兩邊都不能單獨獲勝：
兩公里外但明年三月的，不該贏過三十公里外但明天的；
今晚在島的另一頭的，也不該贏過橋下的市集。

這個誠實的代價是 17 場裡多數永遠不會出現在首頁——台南的活動不在新店附近。
一個三分之二的資料只有站對縣市的人看得到的功能，等於只為那些人存在，
所以那條列表的結尾是一個「看全部活動」磚，
位置與理由跟它上面那條的「看全部導覽」一模一樣。

### 26.6 照片：虛構的東西配真照片，標記反而更重要

17 張 Pexels 圖庫示意圖，角落壓上「圖庫示意」，詳情頁寫出攝影者與**照片實際拍到什麼**。

一張圖庫停車場放在真實的 iRent 據點上，最糟只是「拍錯的停車場」；
一張真實燈節的照片放在一場不會發生的活動上，是把「某件事發生過」的證據
交給一件不會發生的事。alt 屬性寫的是攝影師的描述，不是活動名稱——
alt 描述的是圖片，而圖片不是那場活動，
把虛構的名字寫進螢幕閱讀器唯一當成事實的欄位，是最不該做的一處。
