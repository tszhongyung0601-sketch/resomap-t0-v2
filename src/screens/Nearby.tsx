...
function CategoryCard({
  ...
  return (
    <button
      ...
      disabled={!live}
      className={`min-w-0 flex-1 overflow-hidden rounded-2xl bg-surface text-left transition ${
        live ? "active:bg-surface-2" : "opacity-70"
      }`}
    >
      <div className={live ? "" : "grayscale"}>
        ...
        >
          {/* A floor under the caption, so white text survives a bright
              ...
              without a full-image scrim dulling the picture. Measured against
              the brightest crop in the set rather than guessed. */}
      ...
      {/* The quiet line. Grey, 11px, at the bottom — visible to anybody who
          looks, invisible to anybody who is choosing dinner. */}
      <div className="flex items-center gap-1.5 px-3 py-2">
        <span className="min-w-0 flex-1 truncate text-[11px] text-ink-3">{card.label}</span>
        {live && (
          <span className="shrink-0 text-[12px] text-ink-3" aria-hidden>
            ›
          </span>
        )}
      </div>
